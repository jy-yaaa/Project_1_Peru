...
Description: 	Analyzes coded SUSENAS data and outputs paper Figure 7.
Uses: 			cleandata.dta (Indonesia SUSENAS version)
...
global datapath = "$surveypath/Data to use for analysis"
...
local national_num_households = 61100000
local program_budget_monthly = 24844482000000/12
	//this comes from 30% of 61,100,000 households receiving Rp. equivalent of $100 USD/year
	//then divided by 12 to make sure we are looking at monthly benefit receipt 
...
replace income_percentile = . if missing(lncaphat) // want to exclude missing lncaphat
...
forvalues i = 0/100 { //NOTE: THIS STEP TAKES A LONG TIME. Could abbreviate to i= 0(10)100 for faster runtime
	quietly {
		...
		forvalues j = 0/99 {
			...
			//display number of loops (out of 10,100 total) to track progress
...
gen bonus_perhh = 8700/12
	//this comes from TNP2K document listing per-household cost of 8,700 Rp. / household / year 
...
*** 5. OUTPUT FIGURES ***
//Note: in this file, we only output Figure 7, which is specific to Indonesia
cd "$figurepath"
...
//Figure 7: Expected tax rate given 1.5x poverty line cutoff 
gen receive_benefit = (percapitahat < povertyline150pct) if !missing(percapitahat)


//fit lpoly function to (estimated) benefit receipt status  
lpoly receive_benefit percapitaconsumption if sample_use == 1 & !missing(percapitahat), bw(50000) generate(prob_receive_benefit) at(percapitaconsumption)
...
//benefit amount: $100 USD per household per year 
gen benefit_amount = 112950 if !missing(derivative_prob_receive)
...
gen receive_benefit_morenoise = (percapitahat_morenoise < povertyline150pct) if !missing(percapitahat_morenoise)

//fit lpoly function to (estimated) benefit receipt status  
lpoly receive_benefit_morenoise percapitaconsumption if sample_use == 1 & !missing(percapitahat_morenoise), bw(50000) generate(prob_receive_benefit_morenoise) at(percapitaconsumption)
//take first derivative numerically
...
twoway (lpoly tax percapitaconsumption if sample_use == 1, bw(50000)) (lpoly tax_morenoise percapitaconsumption if sample_use == 1, bw(50000)), ///
 		ytitle(Implied Tax Rate) yline(0, lwidth(medthin) lcolor(black)) xtitle(Monthly per-capita household consumption (Rp.)) title(Indonesia) legend(order(1 "Normal OLS Prediction" 2 "OLS Prediction with Doubled Noise")) graphregion(color(white))

graph export "Figure 7 Indonesia normal and added noise_whitebg.png", replace 
graph export "Figure 7 Indonesia normal and added noise_whitebg.eps", replace 
graph export "Figure 7 Indonesia.eps", replace 
